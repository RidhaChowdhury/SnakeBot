...
public class Snake extends JPanel implements Runnable {
    ...
    static final int MAX_ENERGY = 2000;
    ...
    int[][] grid;
    public static List<Point> snake, treats;
    Font smallFont;
    ...
    public Snake() {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (gameOver) {
                	if(e.getY()>230 && e.getY()<240)	{
                		startNewGame();
	                    repaint();
	                    if(botPlaying)	{
	                    	try {
								theodore = new SnakeBot();
                	...
                	if(e.getY()>240 && e.getY()<260)	{
	                    System.out.print("bot added");
    ...
    void drawStartScreen(Graphics2D g) {
        ...
        g.setFont(smallFont);
        g.drawString("(click here to start)", 240, 240);
        if(!botPlaying)
        	g.drawString("(click here to add bot)", 220, 260);
        else
        	g.drawString("(click here to remove bot)", 200, 260);
    }
